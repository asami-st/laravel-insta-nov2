...
class CategoriesController extends Controller
{
    ...
    public function index(){
        ...
        foreach ($all_posts as $post) {
            if ($post->categoryPost->count() == 0) {
                $uncategorized_count++;  //if the post doesn't have any category on it, then increment the $uncategorized_count property
            }
        }
    ...
    public function store(Request $request){
        ...
        $request->validate([
            'name'  => 'required|min:1|max:50|unique:categories,name'  //categories form table name
        ]);
